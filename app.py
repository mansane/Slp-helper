...
def get_coords_and_code_pure(address):
    ...
    # [수정 1] User-Agent를 추가해서 크롬 브라우저인 척 위장합니다.
    headers = {
    ...
    try:
        # [수정 2] timeout을 5초에서 20초로 대폭 늘려줍니다.
...
with col2:
    ...
    if clicked and df_view is not None:
        ...
        if not detail.empty:
            ...
            with d_col2:
                st.link_button("네이버 부동산",
                               f"https://m.land.naver.com/search/result/{urllib.parse.quote(row['umdNm'] + ' ' + row['단지명'])}")
